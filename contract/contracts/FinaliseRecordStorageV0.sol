...
contract FinaliseRecordStorageV0 {
	...
	function ResetSubmitEntries() public {
		_submit_list.length = 0;
	}
	function PushSubmitEntry(string action, bytes32 hash_value) public {
	...
	function UpdateFinaliseHashMap(bytes32 key, bool existed, bool finalised) public {
		_finalize_hash_map[key].is_exist = existed;
		_finalize_hash_map[key].is_finalise = finalised;
	}
	function PushFinaliseHashMap(bytes32 key, string action, bytes32 hash_value) public {
	...
	function SetKV2FinaliseHashMap(bytes32 key, bytes32 val) public {
